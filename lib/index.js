...
function cachingReply(reply, token, setInCache) {
  function replyFn(x) {
    setInCache(token, x);
    reply(x);
  }
  replyFn.continue = function continueFn(x) {
    setInCache(token, { continue: x });
    reply.continue(x);
  };
  return replyFn;
}
...
internals.implementation = function (server, options) {
  ...
  var checkCache = options.checkCache;
  var setInCache = options.setInCache;

  if ((checkCache && !setInCache) ||
      (setInCache && !checkCache)) {
    throw new Error('if checkCache or setInCache is defined, both must be defined');
  }
  ...
  return {
    authenticate: function (request, reply) {
      ...
      if (checkCache) {
        var cachedReply = checkCache(token);
        if (cachedReply) {
          if (cachedReply.continue) {
            return reply.continue(cachedReply.continue);
          }
          return reply(cachedReply);
        }
        reply = cachingReply(reply, token, setInCache);
      }
